 ...
 *
 * Prevents Wellington from running on WordPress versions prior to 4.7,
 ...
 * Wellington on WordPress versions prior to 4.7.
 *
 * @global string $wp_version WordPress version.
 */
function wellington_upgrade_notice() {
	$message = sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'wellington' ), 'Wellington', '4.7', $GLOBALS['wp_version'] );
...
function wellington_customize() {
	wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'wellington' ), 'Wellington', '4.7', $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function wellington_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'wellington' ), 'Wellington', '4.7', $GLOBALS['wp_version'] ) );
	}
}
