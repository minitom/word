 ...
 */
function wellington_customize_register_magazine_widget_area_control( $wp_customize ) {
	...
	// Create New Control.
	$magazine_control = new Wellington_Magazine_Widget_Area_Customize_Control( $wp_customize, $magazine_setting_id, array(
	...
	$wp_customize->selective_refresh->add_partial( $magazine_setting_id, array(
		...
		'render_callback'     => 'wellington_customize_magazine_placeholder',
		'container_inclusive' => true,
	) );
}
add_action( 'customize_register', 'wellington_customize_register_magazine_widget_area_control' );


/**
 * Displays a Placeholder for adding Magazine widgets.
 */
function wellington_customize_magazine_placeholder() {
	// Show Magazine Widget Placeholder only in Customizer preview.
	if ( is_customize_preview() ) {
		echo '<div id="magazine-placeholder" class="magazine-widgets-placeholder type-post"><span class="magazine-widgets-placeholder-title">' . esc_html__( 'Add Magazine Widget', 'wellington' ) . '</span></div>';
	}
}
